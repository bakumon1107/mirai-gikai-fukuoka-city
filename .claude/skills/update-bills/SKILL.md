...
3. `--dry-run` で件数と切り出し結果を確認し、生成内容をユーザーに提示する:
   ```bash
   ...
   ```
4. レビューを通ったら `--dry-run` を外して実行し、DBに反映する
...
- 議案質疑（第1日の質疑要約）は `packages/seed/fukuoka/import-bill-discussions.ts` を使用（上記「議案質疑（定例会第１日）」参照）
