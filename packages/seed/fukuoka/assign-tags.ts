 ...
 *   例: tsx --env-file=../../.env packages/seed/fukuoka/assign-tags.ts r7-4
 */
 ...
 * 複数タグ可。
 */
// 会期ごとの議案番号 → タグラベルマッピング
const BILL_TAG_MAP_BY_SESSION: Record<string, Record<string, string[]>> = {
  "r7-4": {
    // 財政・予算 —— 令和6年度各会計歳入歳出決算
    "第145号": ["財政・予算"],
    "第146号": ["財政・予算"],
    "第147号": ["財政・予算"],
    "第148号": ["財政・予算"],
    ...
    "第170号": ["財政・予算"],
    // 条例改正
    "第171号": ["福祉・医療"],       // 老人いこいの家条例
    "第172号": ["まちづくり・環境"], // 建築関係手数料条例
    "第173号": ["まちづくり・環境"], // 市営住宅条例
    "第174号": ["子育て・教育"],     // 公民館条例
    "第175号": ["まちづくり・環境"], // 町の区域変更
    // 第176〜187号は名前パターン（契約・和解）で自動付与
    "第183号": ["子育て・教育"],     // 小学校校舎の取得
    "第184号": ["子育て・教育"],     // 小学校増築校舎の取得
    "第185号": ["まちづくり・環境"], // 都市公園区域廃止
  },
};
...
async function main() {
  ...
  const effectiveTagMap = BILL_TAG_MAP_BY_SESSION[slug] ?? BILL_TAG_MAP;

  for (const bill of bills) {
    const tagLabels = effectiveTagMap[bill.bill_number] ?? [];
